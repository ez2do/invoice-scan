...
// NewErrGroupWithRecoveryAndSharedMutex
// @example
// g := NewErrGroupWithRecoveryAndSharedMutex()
//
	...
	g.parent.Go(func() error {
		defer func() {
			if p := recover(); p != nil {
				...
				log.Errorw("ErrGroupWithRecovery panic", "error", g.err)
				g.ctxCancel()
			}
		}()
...
func GoWithRecovery(f func()) {
	go func() {
		defer func() {
			if p := recover(); p != nil {
				log.Errorw("GoWithRecovery: panic", "error", fmt.Sprintf("%v", p))
			}
		}()
